...
MAX_RATE_LIMIT_RETRIES = 10  # cap for 429 retries (~10 min max wait)
...
class AIAgent:
    ...
    def _prune_history(self):
        """Drop old messages to stay under the TPM limit.

        Only prunes at a 'user' turn that is NOT immediately followed by a
        'tool' message, so we never break a tool-call chain that the API
    ...
    def _is_hallucinated_tool_call(self, content: str, allowed: set) -> bool:
        """Return True if the model printed a tool call as raw text instead of using the API."""
        ...
        if "function=" in content and any(name in content for name in allowed):
            return True
        # Raw tool name at start of response
    ...
    def chat(self, user_input: str) -> str:
        ...
        model_retry_count = 0  # hallucination / syntax errors — capped at MAX_RETRIES
        rate_limit_retries = (
            0  # 429 errors — capped separately at MAX_RATE_LIMIT_RETRIES
        )
            ...
            try:
                ...
                if "rate_limit_exceeded" in error_str or "429" in error_str:
                    if rate_limit_retries >= MAX_RATE_LIMIT_RETRIES:
                        ...
                        return "Rate limit is still active after several retries. Please wait a minute and try again."

                    logging.warning(f"Rate limit error: {error_str}")

                    # Use retry-after header if available, else parse error text, else default 62s
                    wait_seconds = 62.0
                    try:
                ...
                # --- Model decommissioned or unknown error ---
